...
export function ClaimReportModal({ s }: { s: AuditorState }) {
  ...
  // Sync local state when extracted data changes
  useEffect(() => {
    setPatientName(s.patientName || 'N/A');
    setHospitalName(s.hospitalName || 'N/A');
    setAdmissionDate(s.admissionDate || 'N/A');
    setDischargeDate(s.dischargeDate || 'N/A');
    setDiagnosis(s.diagnosis || 'N/A');
    setBilledAmount(s.total || 0);
    setExpenses(
      s.lineItems.map((li, i) => ({ id: li.id ? String(li.id) : `exp-${i}`, category: li.category, amount: li.amount }))
    );
  }, [s.realPreview, s.claimId]);
  ...
  const riskScoreNum = summary?.risk_score !== undefined && summary?.risk_score !== null ? Math.round(summary.risk_score * 100) : 0;
